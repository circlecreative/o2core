...
abstract class LibraryInterface extends Glob
{
	...
	public function __reconstruct( array $config = [ ] )
	{
		...
		{
			\O2System\Core::Exceptions()->addPath( dirname( $drivers_path ) );
			\O2System\Core::$language->addPath( dirname( $drivers_path ) )->load( get_class_name( $this ) );
		}
	...
	final private function __getDriversPath()
	{
		...
		$parent_class = $parent_class === 'O2System\Core\Interfaces\LibraryInterface' ? get_class( $this ) : $parent_class;
	...
	final public function setConfig( $config, $value = NULL )
	{
		...
		elseif ( isset( $this->_config[ $config ] ) )
		{
			if ( is_array( $value ) )
			{
				$this->_config[ $config ] = array_merge( $this->_config[ $config ], $value );
			}
			else
			{
	...
	final public function throwError( $error, $code = 0, $vars = [ ], $class_name = NULL )
	{
		...
		{
			$parent_class = get_parent_class( $this );
			$parent_class = $parent_class === 'O2System\Core\Interfaces\Libraries' ? get_class( $this ) : $parent_class;
